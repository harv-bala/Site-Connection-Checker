...
class DBHelper():
    '''executes database operations for easier higher-level use'''
    def __init__(self) -> None:
        # Set the location of the database, as well as the filename. The Path
        # class from pathlib automatically corrects a path based on the OS.
        # On Windows, this would change the '/' to a '\'.
        self.db_path = Path('db/websiteDB.db')

        # Set the SQL pattern for creation of the 'websites' table.
        # Status is set as an integer because SQLite does not have a Boolean
        # data type, so 1 and 0 act as True and False. 'domain' refers to
        # the domain name of the website, e.g. https://www.google.com
        # 'status' refers to whether the website is live or down.
        # 0 = Down, 1 = Live
        ...
        # General SQL pattern for inserting a record.
        # Using .format(), the gaps '{}' can be filled in
        ...
        # If a website's status changes, this SQL will
        # change the status for the domain name given
    ...
    def create_table(self):
        '''create a cursor to access records in the database'''
        connection = self.create_connection()
        # The cursor allows iteration over the records in the database,
        # and handles the execution of any SQL
    ...
    def create_record(self, domain):
        ...
        cursor = connection.cursor()
        try:
            # .format() is used to insert the arguments
            # into the pre-set SQL pattern
    ...
    def read_record(self, domain):
        '''read information from a record in the database'''
        connection = self.create_connection()
        cursor = connection.cursor()
        result = None
        try:
    ...
    def read_all_records(self):
        '''read information from all records in the database'''
        connection = self.create_connection()
        cursor = connection.cursor()
        result = None
        try:
